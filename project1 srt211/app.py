    ...
    try:
        ...
        cursor.execute('''
            ...
            GROUP BY p.id, p.seller_id, p.name, p.description, p.price, p.quantity, 
                     p.total_revenue, p.min_bid, p.bidding_deadline, p.purchased
        ''', (buyer_id,))  # Pass the buyer's user ID to filter their bids
...
def view_bids():
    try:
        ...
        cursor.execute('''
            SELECT p.id, p.seller_id, p.name, p.description, p.price, p.quantity, 
                   ...
                   COALESCE(MAX(CASE WHEN b.buyer_id = %s THEN b.bid_price ELSE NULL END), 0) AS buyer_highest_bid
            FROM products p
            INNER JOIN bids b ON p.id = b.product_id AND b.buyer_id = %s
            GROUP BY p.id, p.seller_id, p.name, p.description, p.price, p.quantity, 
                     p.total_revenue, p.min_bid, p.bidding_deadline, p.purchased
        ''', (buyer_id, buyer_id))
    ...
    #     # Fetch bid data for the buyer if there are bids for the products
    #     for product in products:
    #         if product[0] == product[1]:  # If product ID matches seller ID, implying it's a bid
    #             cursor.execute('''
    #                 SELECT id, product_id, buyer_id, bid_price
    #                 FROM bids
    #                 WHERE product_id = %s
    #             ''', (product[0],))
    #             bid_data = cursor.fetchall()
    #             product['bid_data'] = bid_data
    ...
    #             IF(b.bid_price = MAX(b2.bid_price) AND NOW() > p.bidding_deadline, 'Yes', 'No') AS winning_bid
    #         FROM 
    #             products p
    #         LEFT JOIN 
